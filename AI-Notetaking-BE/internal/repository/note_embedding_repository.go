...
type INoteEmbeddingRepository interface {
	...
	SemanticSearch(ctx context.Context, embedding []float32) ([]*entity.NoteEmbedding, error)
	DeleteByNotebookId(ctx context.Context, notebookId uuid.UUID) error
	SearchSimilarity(ctx context.Context, embedding []float32) ([]*entity.NoteEmbedding, error)
}
...
func (n *noteEmbeddingRepository) DeleteByNotebookId(ctx context.Context, notebookId uuid.UUID) error {
	_, err := n.db.Exec(
		...
		SET is_deleted = TRUE,
			deleted_at = $1
		WHERE note_id IN (
			SELECT id FROM note WHERE notebook_id = $2 AND is_deleted = FALSE
		)`,
		time.Now(),
		notebookId,
	)
...
func (n *noteEmbeddingRepository) SemanticSearch(ctx context.Context, embedding []float32) ([]*entity.NoteEmbedding, error) {
	rows, err := n.db.Query(
		ctx,
...
func (n *noteEmbeddingRepository) SearchSimilarity(ctx context.Context, embedding []float32) ([]*entity.NoteEmbedding, error) {
	rows, err := n.db.Query(
		ctx,
